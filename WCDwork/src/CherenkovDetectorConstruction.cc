...
G4VPhysicalVolume* CherenkovDetectorConstruction::Construct()
{
  ...
  //HARD-CODED (Dynamic later) (ask for input?)
  //G4double delta = 0.1;
  //G4double r_s = 1000; //Suppose tank is at 1000m for now.
  // delta*r_s
  ...
  G4VPhysicalVolume* tank_phys = new G4PVPlacement(0,G4ThreeVector(r_s,0,0), tank_log, "tank", world_log, false, 0);
  ...
  G4VPhysicalVolume* watertank_phys = new G4PVPlacement(0,G4ThreeVector(r_s,0,0), watertank_log, "watertank", world_log, false, 0);
